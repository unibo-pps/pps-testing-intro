...
class TestOnLists extends Properties("Seqs"):
  ...
  property("Reverse") = forAll((l1: Seq[Int], l2: Seq[Int]) => l1.reverse.reverse == l1)
end TestOnLists
...
class TestPersons extends Properties("Persons"):
  ...
  given arbitraryPerson: Arbitrary[Person] = Arbitrary(for {
    name <- Arbitrary(Gen.oneOf("Bob", "Rick")).arbitrary
  ...
  property("Adultness 2") = forAll((p: Person) => Prop.collect(p)(p.adult))
end TestPersons
